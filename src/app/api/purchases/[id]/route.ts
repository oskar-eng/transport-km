...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const po = await prisma.purchaseOrder.update({
    where: { id },
    data: { ...(body.status ? { status: body.status } : {}), ...(body.receiptUrl !== undefined ? { receiptUrl: body.receiptUrl || null } : {}) },
    include: { items: true },
  });
  return NextResponse.json({ ...po, date: po.date.toISOString(), createdAt: po.createdAt.toISOString(), updatedAt: po.updatedAt.toISOString() });
}
